...
* **Dependency Updates** - Bumped `ruff` (`0.15.22` → `0.16.6`), `basedpyright` (`1.39.9` → `1.40.0`), `pip` (`26.1.2` → `26.2.1`), `github-action-toolkit` (`0.9.0` → `0.10.0`), `splunk-add-on-ucc-framework` (`6.5.2` → `6.6.0`), `splunk-appinspect` (`4.2.1` → `4.3.1`), `sphinx-autodoc-typehints` (`3.13.0` → `3.13.5`), and `sphinxcontrib-mermaid` (`2.1.0` → `2.1.1`), and refreshed `uv.lock`.
...
* **Supply-chain Hardening** - Pinned all external Github Actions (`actions/checkout`, `astral-sh/setup-uv`, `actions/upload-artifact`, `softprops/action-gh-release`, `VatsalJagani/pytest-cov-action`) to full commit SHAs with a `# <version>` comment across the workflows and `action.yml`, per Github's recommended hardening. Also aligned `action.yml`'s `setup-uv` reference (was still `v5`) to `v9.0.0`. Dependabot's `github-actions` ecosystem keeps the SHAs and version comments up to date automatically, so no config change was needed. Self-references (`VatsalJagani/splunk-app-action@develop`) and local reusable-workflow calls are intentionally left unpinned.
* **Dependabot** - Switched the Python ecosystem from `pip` to the native `uv` ecosystem (correctly updates `uv.lock` for this uv-managed project), moved to a weekly schedule, and grouped updates per ecosystem with PR limits. Added an `ignore` rule for `python-magic`, which is hard-pinned to `==0.4.24` by `splunk-appinspect 4.2.1`; without it Dependabot repeatedly failed the update job with a `dependency_file_not_resolvable` error trying to bump `python-magic`.
