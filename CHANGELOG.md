...
* Added `is_remove_not_allowed_executables_from_lib` input (default: `true`) to control removal of files with mimetype `application/x-executable` or `application/x-sharedlib` from UCC-generated `lib/` before packaging. This will help with Splunk App-inspect failures.
