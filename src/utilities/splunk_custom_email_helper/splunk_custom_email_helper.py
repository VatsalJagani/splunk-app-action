...
class SAKEmailUtility:
    ...
    def savedsearch_level_overridden_email_configs(self, alert_name):
        _, serverContent = rest.simpleRequest(
            "/servicesNS/-/{}/saved/searches/{}?output_mode=json".format(sak_utils.APP_NAME, quote(alert_name, safe='')),
            method='GET', sessionKey=self.session_key, raiseAllErrors=True)
    ...
    def send(self, to, cc=[], bcc=[], subject='SAK Alert', html_body=''):
        # Define Email
        ...
        html_footer_content = SAKEmailHTMLBodyBuilder.htmlFooterTemplate().render(footer='This email is generated by Swiss Army Knife App for Splunk Admins. (Build By: CrossRealms International)', re=re, filters=filters)
        full_html_body = SAKEmailHTMLBodyBuilder.htmlRootTemplate().render(body=html_body, footer=html_footer_content)
