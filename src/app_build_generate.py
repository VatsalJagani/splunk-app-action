...
def file_folder_permission_changes():
    if gat.get_user_input_as("to_make_permission_changes", bool, False):
        # Permission Changes
        os.system(
...
def generate_build(app_build_dir_name, app_build_dir_path):
    gat.info(f"Generating the app build., app_dir_path={app_build_dir_path}, app_package_id={GlobalVariables.APP_PACKAGE_ID}, app_version_encoded={GlobalVariables.APP_VERSION_ENCODED}, app_build_number_encoded={GlobalVariables.APP_BUILD_NUMBER_ENCODED}")
