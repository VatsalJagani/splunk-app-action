...
def main():
    gat.info("Starting splunk-app-action")
    gat.print_all_user_inputs()

    GlobalVariables.initiate(app_dir_name=gat.get_user_input('app_dir'))
    ...
    if use_ucc_gen:
        ...
        app_package_id = splunk_app_details.fetch_app_package_id_from_app_conf(
            app_conf_file_path, gat.get_user_input('app_dir')
        )
    ...
    if use_ucc_gen:
        ...
        _ret_code = os.system(f"rm -rf {app_build_dir_name}")
        
        if _ret_code != 0:
            raise Exception("Unable to remove the app_build_dir")
    ...
    try:
        ...
        gat.error("Error Adding Splunk App Utilities: {}".format(e))
    ...
    try:
        ...
        # Run App Inspect
        if gat.get_user_input_as('is_app_inspect_check', bool, True):
