...
class TestWhatsInsideTheAppUtility(unittest.TestCase):
    ...
    def test_implement_utility_readme_updated(self):
        with patch.object(
            self.utility, "_get_xml_dashboards", return_value=["Dashboard 1", "Dashboard 2"]
        ):
            with patch.object(
                self.utility, "_get_imp_conf_files_details", return_value=["Details"]
            ):
                with patch.object(
                    self.utility, "_get_csv_lookup_files", return_value=["Lookup Files"]
                ):
                    with patch.object(
                        self.utility, "_get_readme_file_location", return_value="/path/to/README.md"
                    ):
                        with patch(
                            "helpers.file_manager.PartRawFileHandler.validate_file_content",
                            return_value=True,
                        ):
    ...
    def test_get_stanzas(self):
        ...
        with patch("os.path.isfile", return_value=True):
            with patch(
                "utilities.whats_inside_app.WhatsInsideTheAppUtility._get_conf_stanzas",
                return_value=["stanza1", "stanza2"],
            ):
    ...
    def test_get_imp_conf_files_details(self):
        with patch(
            "utilities.whats_inside_app.WhatsInsideTheAppUtility._get_stanzas",
            return_value=["stanza1", "stanza2"],
        ):
    ...
    def test_get_xml_dashboards(self):
        with patch("os.path.isdir", return_value=True):
            with patch("os.listdir", return_value=["dashboard1.xml", "dashboard2.xml"]):
                with patch(
                    "utilities.whats_inside_app.WhatsInsideTheAppUtility._get_xml_dashboard_details",
                    return_value={
                    ...
                    self.assertSetEqual(
                        ...
                        set(
                            [
                                "No of XML Dashboards: **2**",
                                "Approx Total Viz(Charts/Tables/Map) in XML dashboards: **6**",
                            ]
                        ),
                    )
