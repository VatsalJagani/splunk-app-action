    ...
    with get_temp_git_repo() as repo_path:
        with GitHubPR(repo_dir=repo_path) as github_pr:
            ...
            output = subprocess.run(
                ["git", "log", "--oneline", "--grep", "splunk_app_action_my_hash"],
                capture_output=True,
                text=True,
            )
            assert "splunk_app_action_my_hash" in output.stdout.strip()
    ...
    with get_temp_git_repo() as repo_path:
        with GitHubPR(repo_dir=repo_path) as github_pr:
            with stdout_capture() as captured_stdout:
                ...
                assert (
                    'CMD=git commit -m "splunk_app_action_my_hash", ReturnCode=1' in output.strip()
                )
    ...
    with get_temp_git_repo() as repo_path:
        with GitHubPR(repo_dir=repo_path) as github_pr:
            ...
            with stdout_capture() as captured_stdout:
                ...
                output = captured_stdout.getvalue()
                assert (
                    "Branch already present." in output.strip()
                    or "a branch named 'splunk_app_action_my_hash' already exists" in output.strip()
                )
